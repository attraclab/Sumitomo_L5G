#!/bin/bash

rosws=catkin_ws
project_name=Sumitomo_L5G

sleep 35

export DISPLAY=:0.0
export LOGFILE=/home/$USER/$project_name/autostart_scripts/json_ros_converter.log

source /home/$USER/$project_name/autostart_scripts/ROS_CONFIG.txt
source /opt/ros/melodic/setup.bash
source /home/$USER/$rosws/devel/setup.bash

cd /home/$USER/web_dev/sumitomo_l5g/python/

while true
do
		echo >>$LOGFILE
		echo "----------------------------------------------" >> $LOGFILE
		date >> $LOGFILE

		echo "Starting python json_ros_converter.py" >> $LOGFILE
		
		python -u json_ros_converter.py >> $LOGFILE

		echo "program seems to have stopped" >> $LOGFILE

		date >> $LOGFILE
		sleep 1

done
